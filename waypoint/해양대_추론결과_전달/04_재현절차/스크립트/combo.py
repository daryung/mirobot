...
RUNS = [
    ('N16+G75',      dict(npts=16384, batch=2, grid=0.0075, margin=80.0)),
    ('N16+CROP',     dict(npts=16384, batch=2, grid=0.01,   margin=30.0)),
    ('N16+G75+CROP', dict(npts=16384, batch=2, grid=0.0075, margin=30.0)),
    ('합동(NIA+해양대)', dict(npts=8192, batch=4, grid=0.01, margin=80.0, joint=True)),
]
summary = []
for tag, cfg in RUNS:
...
with open(os.path.join(RES, '해양대66_조합_합동.csv'), 'w', newline='', encoding='utf-8-sig') as f:
